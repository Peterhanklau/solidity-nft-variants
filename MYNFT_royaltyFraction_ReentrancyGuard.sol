...
contract MyNFT is ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    // 设置版税信息
    function setRoyaltyInfo(address receiver, uint96 feeNumerator) external onlyOwner nonReentrant {
    ...
    function mintNFT(address recipient, string memory tokenURI) public nonReentrant returns (uint256) {
    ...
    function bulkMint(uint256 count) external payable nonReentrant returns (uint256[] memory) {
        require(whitelist[msg.sender],"MyNFT: Caller is not in the whitelist");
