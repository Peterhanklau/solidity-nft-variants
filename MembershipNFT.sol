...
contract FitClubMembership is ERC721, Ownable {
    ...
    /// @notice 续费会员
    /// @param durationInDays 续费时长（天数）
    function renewMembership(uint256 durationInDays) external payable {
    ...
    function getMembershipInfo(address user) external view returns (MembershipInfo memory info) {
        ...
        if (tokenId == 0) {
            ...
            return MembershipInfo({
                ...
                isActive: false,
                tokenId: 0,
                level: MembershipLevel(0), // 使用默认枚举值
                expirationDate: 0,
                discountRate: 0,
                daysRemaining: 0
            });
    ...
    /// @return discountRate 折扣率（如80表示8折）
    /// @return isValid 是否有效
    ...
    /// @param level 会员等级
    /// @return price 价格（wei）
    function getLevelPrice(MembershipLevel level) external view returns (uint256) {
        return _levelPrices[level];
    }
    
    /// @notice 获取合约信息
    /// @return totalMembers 会员总数
    /// @return contractBalance 合约余额
    /// @return marketplace 官方市场地址
    function getContractInfo() external view returns (
        uint256 totalMembers,
        uint256 contractBalance,
        address marketplace
    ) {
        return (totalSupply(), address(this).balance, _officialMarketplace);
    }
    ...
    function _generateTokenURI(uint256 tokenId) internal view returns (string memory) {
        ...
        string memory svg = string(abi.encodePacked(
            ...
            '<defs><linearGradient id="grad" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stop-color="#1a1a2e"/><stop offset="100%" stop-color="#16213e"/></linearGradient></defs>',
            '<rect width="400" height="450" fill="url(#grad)" rx="20" ry="20"/>',
            '<rect x="20" y="20" width="360" height="410" fill="rgba(255,255,255,0.05)" rx="15" ry="15"/>',
            
            // 标题
            '<text x="200" y="70" text-anchor="middle" fill="#ffffff" font-family="Arial, sans-serif" font-size="28" font-weight="bold">FitClub Membership</text>',
            
            // 会员等级
            '<text x="200" y="120" text-anchor="middle" fill="#4cc9f0" font-family="Arial, sans-serif" font-size="32" font-weight="bold">', levelName, '</text>',
            
            // 会员编号
            '<text x="200" y="160" text-anchor="middle" fill="#cccccc" font-family="Arial, sans-serif" font-size="16">ID #', tokenId.toString(), '</text>',
            
            // 折扣信息
            '<text x="200" y="220" text-anchor="middle" fill="#f72585" font-family="Arial, sans-serif" font-size="24" font-weight="bold">',
            'Save ', _toString(100 - member.discountRate), '%</text>',
            
            // 状态
            '<text x="200" y="270" text-anchor="middle" fill="#', statusColor, '" font-family="Arial, sans-serif" font-size="20" font-weight="bold">', status, '</text>',
            
            // 过期时间
            '<text x="200" y="320" text-anchor="middle" fill="#ffffff" font-family="Arial, sans-serif" font-size="16">Expires: ', expiryDate, '</text>',
            
            // 剩余天数
            '<text x="200" y="350" text-anchor="middle" fill="#', statusColor, '" font-family="Arial, sans-serif" font-size="16">',
            daysRemaining.toString(), ' days remaining</text>',
            
            // 折扣率说明
            '<text x="200" y="390" text-anchor="middle" fill="#aaaaaa" font-family="Arial, sans-serif" font-size="14">',
            'Presents this NFT for ', _toString(member.discountRate), '% discount rate</text>',
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked(
            ...
            '", "description": "Official FitClub Gym Membership NFT. This digital membership card grants access to exclusive benefits, services, and discounts at all FitClub locations.",
            '"image": "data:image/svg+xml;base64,', Base64.encode(bytes(svg)),
            '", "attributes": [',
            '{"trait_type": "Level", "value": "', levelName, '"},',
            '{"trait_type": "Discount Rate", "value": "', _toString(member.discountRate), '%"},',
            '{"trait_type": "Status", "value": "', status, '"},',
            '{"trait_type": "Expiration Date", "display_type": "date", "value": "', _toString(member.expirationDate), '"},',
            '{"trait_type": "Days Remaining", "value": "', daysRemaining.toString(), '"},',
            '{"trait_type": "Purchase Price", "display_type": "number", "value": "', _toString(member.purchasePrice), '"}',
            ']}'
        ))));
    ...
    function _getStatusColor(string memory status) internal pure returns (string memory) {
        ...
        if (statusHash == keccak256(abi.encodePacked("Active"))) return "4cc9f0";
        if (statusHash == keccak256(abi.encodePacked("Expiring Soon"))) return "f8961e";
        return "f72585"; // Expired
    }
    
    /// @notice 格式化时间戳
    function _formatTimestamp(uint256 timestamp) internal pure returns (string memory) {
        // 简化版本 - 生产环境建议在前端处理日期格式化
